     ...
     class congThuc{
        function themCT($maCT ,$ten, $donVi, $soLuong1, $DSNL,$moTa1){
            ...
            if(!$db){
                ...
                if( $ten !=""&& $donVi !=""&& $soLuong1!=""&& $moTa1 !="" &&$DSNL!="") {
                        ...
                        $query = "insert congthuc values($maCT ,$ten, $donVi, $soLuong1, $DSNL,$moTa1)";
        ...
        function suaCT($ten, $donVi, $soLuong1, $DSNL,$moTa1){
                ...
                if(!$db){
                    ...
                    if( $ten !=""&& $donVi !=""&& $soLuong1!=""&& $moTa1 !="" &&$DSNL!="") {
                        $query = "UPDATE congthuc SET Ten=$ten, DonVi= $donVi, SoLuong= $soLuong1,DSNL= $DSNL,MoTa = $moTa1";
                        $res = mysqli_query($conn,$query);
                        $p->dongKetNoi($conn);
                        if(!$res){
                            return false;
            ...
            function xemCT($maCT){
                ...
                if(!$db){
                    ...
                    $query = "SELECT * FROM where MaCT=$maCT";
                    $res = mysqli_query($conn,$query);
                    $p->dongKetNoi($conn);
                    if(!$res){
                        return false;

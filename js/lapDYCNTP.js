...
function render(dsThanhPham = null) {
  let html = dsThanhPham !== null ? content(dsThanhPham) : content();
  html = `${menu()}
      ${html}
      `;
...
function content(dsThanhPham = null) {
  let html = `<div class="content">
        <h5>Lập đơn yêu cầu > Nhập thành phẩm</h5>
        <form class="don">
          <h2 class = "tittle">ĐƠN YÊU CẦU NHẬP THÀNH PHẨM</h2>
          <table>
          <tr>
...
function init() {
  ...
  const buttonLD = document.querySelector("#lapDonTP");
...
function themInput() {
  return `<div class="inputInfo dsThanhPham row">
              <table>
                <tr>
                    <td><select name="thanhPham" class="thanhPham">
                  ...
                  /></td>
                    ...
                    <td><input type="date" class="" name="ngaySX" /></td>
                    <td><input type="date" class="" name="ngayHH" /></td>
                </tr>
            </table>
            </div>`;
}
  ...
  lapDonTP.addEventListener("click", (e) => {
    ...
    chonNguyenLieu.forEach((thanhPham, i) => {
      ...
      if (ten != "default" && soluong != "") {
        ...
        let html = `Vui lòng chọn đầy đủ thông tin`;

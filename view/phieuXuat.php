...
<body>
  <div class="section">
    ...
    <div class="phai">
      <div class="phanphoi">
        <a href="#">
          <h3>Đơn yêu cầu > Đơn yêu cầu xuất</h3>
        </a>
      </div>
      <div class="khung">
        <table class="table">
          ...
          $sql = "SELECT * FROM donyeucau WHERE MaLoai IN ('2', '4')";
          $result = $conn->query($sql);

          while ($row = $result->fetch_assoc()) {
            echo
            "<tr onclick=\"window.location='chiTietPhieuXuat.php?MaDon=" . $row['MaDon'] . "'\">
      <td>" . $row['MaDon'] . "</td>
      <td>" . $row['NgayLap'] . "</td>
      <td>" . $row['MaLoai'] . "</td>
      <td>" . $row['SoLuong'] . "</td>
